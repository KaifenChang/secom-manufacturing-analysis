...
# 4_Feature importance using XGBoost

print("\nPerforming XGBoost Feature Importance Analysis...")

# 4_1 Prepare and train the data
# Update numeric_cols to reflect the columns present in df_reduced
...
print(f"\nClass distribution - Negative: {len(y[y == 0])}, Positive: {len(y[y == 1])}")
...
# Initialize and train XGBoost with better parameters
xgb = XGBClassifier(
    n_estimators=200,
    learning_rate=0.1,
    max_depth=5,
    ...
    random_state=42,
    scale_pos_weight=scale_pos_weight,  # Add calculated weight
    eval_metric='auc'  # Add evaluation metric
)
...
# Get feature importance scores using feature_importances_ attribute
...
print(f"Number of features with non-zero importance: {len(feature_importances[feature_importances['Importance'] > 0])}")
print("\nTop 5 features account for {:.2f}% of total importance".format(
    feature_importances['Importance_Percentage'].head(5).sum()))

# Plot feature importance
plt.figure(figsize=(12, 8))
plt.barh(feature_importances['Feature'].head(20), 
         feature_importances['Importance_Percentage'].head(20))
...
print("\nFeature importance results saved to 'feature_importance.csv'")

# drop features with importance less than 0.01
features_to_keep = feature_importances[feature_importances['Importance'] > 0.01]['Feature'].tolist()

# Keep only the important features in the final dataset
df_final = df_reduced[features_to_keep + ['Label']]
...
# Plot feature correlation heatmap with the important features
plt.figure(figsize=(10, 8))
sns.heatmap(df_final[numeric_cols].corr(), annot=True, cmap="coolwarm")
plt.title("Feature Correlation Heatmap (Important Features)")
plt.tight_layout()
#plt.show()
